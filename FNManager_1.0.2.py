...
6 Cut file names to last delimiter
7 Cut file names until delimiter
8 Move all to upper dir
9 Exit
...
while True:
    ...
    if input_num == '1': # set working dir
    ...
    elif input_num == '3': # check the file/folder list
    ...
    elif input_num == '6': # Cut file names to last delimiter
        print(prt3)
        delimiter = str(input())
        if delimiter == '.':
            print("'.'is unavailable")
    ...
    elif input_num == '7': # Cut file names until delimiter
        ...
        # else:
        #     for f in files:
        #         newname = f.replace(f, f.split('%s' %delimiter)[0])
        #         os.rename(f, newname)


    elif input_num == '8': # move all to upper dir
        for f in files: shutil.move(f, '..\\')

    elif input_num == '9': break
